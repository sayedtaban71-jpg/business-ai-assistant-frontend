...
interface AppStateStore extends AppState {
	...
	cloneDashboard: (id: string, newName?: string) => string;
	...
	// Tile positions
	tilePositionsById: Record<string, { x: number; y: number; width: number; height: number }>;
	setTilePositionForTileId: (tileId: string, position: { x: number; y: number; width: number; height: number }) => void;
...
export const useAppState = create<AppStateStore>((set, get) => ({
	...
	tilePositionsById: (() => {
		if (typeof window === 'undefined') return {} as Record<string, { x: number; y: number; width: number; height: number }>;
		try {
	...
	updateTile: async (tileId, updates) => {
		try {
			...
			// const updatedTile = await updateTileDB(tileId, updates);
	...
	cloneDashboard: (id, newName) => {
		...
		const newId = get().createDashboard(name);
		// Clone appearance
		const appearance = { ...get().appearanceByDashboard };
		if (appearance[id]) {
	...
	dashboardTileIds: {} as Record<string, string[]>,
	...
	setTilePositionForTileId: (tileId, position) => {
		const updated = { ...get().tilePositionsById, [tileId]: position };
	...
	companyContextByCompanyId: {} as Record<string, { notes: string; pastedText: string; attachments: { id: string; name: string; type: string; dataUrl: string }[] }>,
