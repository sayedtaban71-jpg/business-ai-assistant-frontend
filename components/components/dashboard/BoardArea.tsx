...
export function BoardArea() {
	const { tiles, currentDashboardId, dashboardTileIds, tileSizeById, tilePositionsById, appearanceByDashboard, setSelectedCompany, reorderTilesInCurrentDashboard, setTilePositionForTileId, companies } = useAppState();
	...
	const currentId = currentDashboardId || 'default';
	...
	useEffect(() => {
		...
		tileIds.forEach((id, index) => {
			const savedPosition = tilePositionsById[id];
	...
	}, [tileIds, tileSizeById, tilePositionsById]);
	...
	useEffect(() => {
		try {
			...
			console.log('Cookie check - companies length:', companies.length);
			console.log('Cookie check - found ID:', last_selected_company_id);

			if (last_selected_company_id && companies.length > 0) {
				console.log('Found company ID in cookie:', last_selected_company_id);
				const company = companies.find(c => c.id === last_selected_company_id);
				if (company) {
					console.log('Setting selected company from cookie:', company.name);
					setSelectedCompany(company);
				} else {
					console.log('Company from cookie not found in companies list, clearing cookie');
	...
	const handleSizeChange = useCallback((tileId: string, size: { width: number; height: number }) => {
		...
		setTilePositionForTileId(tileId, { x: 0, y: 0, ...size });
	}, [setTilePositionForTileId]);
